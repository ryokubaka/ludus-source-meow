...
setup_ansible_env() {
  # Ludus packer runs as Linux user `ludus`; ANSIBLE_HOME is the range owner's
  # tree (/opt/ludus/users/<user>/.ansible). Without ANSIBLE_SSH_CONTROL_PATH_DIR
  # ansible defaults to ~/.ansible/cp (/home/ludus/.ansible/cp) which is RO.
